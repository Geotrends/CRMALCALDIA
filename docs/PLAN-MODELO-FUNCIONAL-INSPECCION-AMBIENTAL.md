...
**Brechas de notificación restantes** (medio/bajo impacto, priorizadas pero no atendidas en este ajuste): art. 172 par. 2 — informar a Policía Nacional al imponer una `MedidaCorrectiva` (`ReporteRNMC` sin recordatorio automático); plazo legal de `RemisionAutoridad` (Ambiental Preventiva); deber de informar al solicitante y programar renovación de `PermisoCaninoManejoEspecial`; alertas de programación de visita (`ProgramacionVisita`, ya prevista como uso de `AlertaProceso` en el propio modelo, BPMN de Visita VIS03); coordinación interna de Gestión Técnica (`IntervencionTecnica`, `RecomendacionTecnica`, `Compromiso`, `VerificacionCumplimiento`, `EvaluacionResultado`) y trazabilidad de `MovimientoExpediente`/`GrabacionAudiencia`/`OrdenPolicia`/`EjecucionMedidaCorrectiva`/`ActuacionMaltratoAnimal`.
